...
using Endava.Hl7.Fhir.Common.Contracts.Services;
using MongoDB.Bson;
...
namespace Endava.Hl7.Fhir.OpenAPI.Controllers.V1
{
    ...
    public class PatientController : BaseController<PatientController>
    {
        ...
        {
            var result = await _mongoDBService.GetAsync(id);
            if(result is null)
            {
                return NotFound();
            }
        ...
        {
            await _mongoDBService.CreateAsync(newPatient);
        ...
        {
            var patient = await _mongoDBService.GetAsync(id);
            if(patient == null)
            {
                return NotFound();
            }
